...
#import "WaterfallDecorationReusableView.h"

NSString* const WaterfallLayoutCellKind = @"WaterfallCell";
NSString* const WaterfallLayouDecorationKind = @"Decoration";

@interface WaterfallCollectionViewLayout()
@property (nonatomic) CGFloat itemWidth;
@property (nonatomic) CGFloat itemInnerMargin; //item 的间距
...
- (void)setup {
    [self registerClass:[WaterfallDecorationReusableView class] forDecorationViewOfKind:WaterfallLayouDecorationKind];
    self.itemWidth = 140.0f;
    self.topInset = 0;
    self.bottomInset = 0;
    self.showHeaderNoneDate = YES;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    return self.layoutInfo[WaterfallLayoutCellKind][indexPath];
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForDecorationViewOfKind:(NSString*)decorationViewKind atIndexPath:(NSIndexPath *)indexPath {
    return self.layoutInfo[WaterfallLayouDecorationKind][indexPath];
}
...
- (void) calculateMaxColumnsCount {
    self.itemWidth = (self.collectionView.frame.size.width - self.sectionInset.left - self.sectionInset.right - (self.columnsCount - 1) * self.itemSpacing)/self.columnsCount;
}

- (void) calculateItemsInnerMargin {
    if(self.columnsCount > 1) {
        self.itemInnerMargin = self.itemSpacing;
    }
}

- (void)setSectionInset:(UIEdgeInsets)sectionInset {
    _sectionInset = sectionInset;
    self.topInset = _sectionInset.top;
    self.bottomInset = _sectionInset.bottom;
}
...
- (void) calculateItemsAttributes {
    ...
    [UICollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:WaterfallLayouDecorationKind
    ...
    for (NSInteger section = 0; section < [self.collectionView numberOfSections]; section++) {
        ...
        if (self.isShowHeaderNoneDate && ([self.collectionView numberOfItemsInSection:section] == 0)) {
            UICollectionViewLayoutAttributes *titleAttributes = [UICollectionViewLayoutAttributes
                                                                 layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader
                                                                 withIndexPath:indexPath];
            titleAttributes.frame = [self frameForWaterfallHeaderAtIndexPath:indexPath];
    ...
    newLayoutInfo[WaterfallLayoutCellKind] = cellLayoutInfo;
    newLayoutInfo[UICollectionElementKindSectionHeader] = titleLayoutInfo;
    newLayoutInfo[WaterfallLayouDecorationKind] = @{indexPath: emblemAttributes};
...
- (CGRect)frameForWaterfallCellIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSInteger column = 0; column < self.columnsCount; column++) {
        columnsHeights[column] = [self headerHeightForIndexPath:indexPath] + (indexPath.item < self.columnsCount ? 0 : self.itemInnerMargin);
    }
...
//        columnsHeights[currentColumn] += (indexPath.item < self.columnsCount ? 0 : self.itemInnerMargin);
    }
    ...
    CGFloat originX = self.sectionInset.left + columnForCurrentItem * self.itemWidth + columnForCurrentItem * self.itemInnerMargin;
...
- (CGRect)frameForWaterfallDecoration {
    CGSize size = [WaterfallDecorationReusableView defaultSize];
